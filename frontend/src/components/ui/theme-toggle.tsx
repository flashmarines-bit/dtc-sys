    ...
    <div className="space-y-3 p-1">
      ...
      <div>
        ...
        <div className="grid grid-cols-5 gap-1.5">
          {COLOR_THEMES.map(t => (
            <button key={t.id} onClick={() => setColorTheme(t.id as ColorThemeId)} title={t.label}
              className={cn(
                "flex flex-col items-center gap-1 p-1.5 rounded-lg border-2 transition-all hover:scale-105",
                colorTheme === t.id ? "border-primary shadow-sm" : "border-transparent hover:border-border"
              )}>
              <div className="w-6 h-6 rounded-full shadow-sm flex items-center justify-center"
                style={{ backgroundColor: t.color }}>
                {colorTheme === t.id && <Check className="w-3 h-3 text-foreground" strokeWidth={3} />}
              </div>
